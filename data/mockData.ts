  ...
  switch (distributionName.split(' ')[0]) {
    ...
    case 'K-均值聚类':
        ...
        for (let i = 0; i < count; i++) {
            ...
            if (type < 0.3) { // High value
                clusters.push({ customerId: `K${4001+i}`, frequency: Math.round(randomNormal(10, 2)), monetary: Math.round(randomNormal(15000, 3000)) });
            } else if (type < 0.7) { // Potential
                clusters.push({ customerId: `K${4001+i}`, frequency: Math.round(randomNormal(4, 1)), monetary: Math.round(randomNormal(5000, 1500)) });
            } else { // Low value
                clusters.push({ customerId: `K${4001+i}`, frequency: Math.round(randomNormal(1.5, 0.5)), monetary: Math.round(randomNormal(1000, 500)) });
            }
        }
        return clusters;
    ...
    case '联合分析':
        const scenarios = [
            { scenario: 1, brand: 'A', price: 10000, duration: '6 months', doctor_exp: '5 years', utility: 4.5 },
            { scenario: 2, brand: 'B', price: 10000, duration: '12 months', doctor_exp: '5 years', utility: 5.2 },
            { scenario: 3, brand: 'A', price: 15000, duration: '12 months', doctor_exp: '10 years', utility: 6.1 },
            { scenario: 4, brand: 'B', price: 15000, duration: '6 months', doctor_exp: '10 years', utility: 4.8 },
        ];
        return scenarios.flatMap(s => Array.from({length: Math.floor(count/4)}, (_, i) => ({...s, customerId: `CJ${1001 + i}`, choice: Math.random() < s.utility/10 ? 1: 0})));

    case '主成分分析':
        return Array.from({length: count}, (_, i) => {
            const q1 = randomNormal(4, 1); const q2 = randomNormal(3, 1);
            const q3 = randomNormal(4.5, 0.8); const q4 = randomNormal(4.2, 1.2);
            return {
                customerId: `PCA${2001+i}`,
                q1_service: parseFloat(q1.toFixed(1)), q2_price: parseFloat(q2.toFixed(1)),
                q3_effect: parseFloat(q3.toFixed(1)), q4_brand: parseFloat(q4.toFixed(1)),
    ...
    case '增益模型':
        return Array.from({length: count}, (_, i) => {
            ...
            let segment = '';
            if (typeRoll < 0.2) segment = 'Persuadable'; // 20%
            else if (typeRoll < 0.6) segment = 'Sure Thing'; // 40%
            else if (typeRoll < 0.9) segment = 'Lost Cause'; // 30%
    ...
    case 'Cox比例风险模型':
        return Array.from({length: count}, (_, i) => {
            ...
            return {
                ...
                discount_usage_rate: parseFloat((high_risk ? randomNormal(0.1, 0.05) : randomNormal(0.4, 0.1)).toFixed(2)),
            };
        });
...
export const convertToCSV = (data: any[]): string => {
  ...
  const rows = data.map(obj => 
    headers.map(header => 
      JSON.stringify(obj[header], (_, value) => value === null ? '' : value)
    ).join(',')
  );
