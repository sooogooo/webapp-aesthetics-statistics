...
const LearningPaths: React.FC<LearningPathsProps> = ({ setCurrentPage, setSelectedId }) => {
  ...
  return (
    <div className="animate-fade-in-up space-y-10">
      <header>
        <h1 className="text-3xl sm:text-4xl font-extrabold text-[color:var(--color-text-base)] tracking-tight">学习路径</h1>
        <p className="text-lg text-[color:var(--color-text-muted)] mt-2">从目标出发，将零散知识串联成解决实际问题的能力。</p>
      </header>

      <div className="flex flex-col md:flex-row gap-8">
        {/* Path Selector */}
        <aside className="md:w-1/3 lg:w-1/4">
          <h2 className="text-lg font-bold text-[color:var(--color-text-base)] mb-4">选择您的角色</h2>
          <div className="space-y-2">
            {learningPathData.map(path => (
              ...
              >
                ...
                <div>
                  <p className="font-semibold text-sm text-[color:var(--color-text-base)]">{path.audience}</p>
        ...
        <main className="flex-1">
          <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-200">
            <h2 className="text-xl font-bold text-[color:var(--color-text-base)]">{activePath.title}</h2>
              ...
              <div className="relative">
                ...
                <div className="space-y-8">
                  {activePath.steps.map((step, index) => {
                    const model = distributionsData.find(d => d.id === step.modelId);
                    return (
                      <div key={index} className="relative flex items-start space-x-4">
                        ...
                        <div className="flex-1 pt-1">
                          <h3 className="font-bold text-[color:var(--color-text-base)]">{step.title}</h3>
                          <p className="text-sm text-[color:var(--color-text-muted)] mt-1 mb-2">{step.description}</p>
                          {model && (
                             <button onClick={() => handleNavigation(model.id, activePath.id, index)} className="flex items-center space-x-1 text-sm font-medium text-[color:rgb(var(--color-primary))] hover:underline">
