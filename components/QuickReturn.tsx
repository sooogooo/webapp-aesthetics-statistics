...
const QuickReturn: React.FC<QuickReturnProps> = ({ currentPage, setCurrentPage, setSelectedId }) => {
  ...
  const lastViewedModelId = useMemo(() => {
    ...
    if (viewedModelIds.length > 0) {
      ...
      const currentModelId = currentPage === 'models' ? viewedModelIds[viewedModelIds.length - 1] : -1;
      ...
      <div className="fixed bottom-[8.5rem] md:bottom-24 right-6 z-50">
        <div className={`flex flex-col items-center transition-all duration-300 ease-in-out`}>
          ...
          <button
            ...
            aria-label="快速返回"
          >
            <span className={`material-symbols-outlined text-3xl transition-transform duration-300 ${isOpen ? 'rotate-45' : 'rotate-0'}`}>
                add
            </span>
          </button>
        </div>
