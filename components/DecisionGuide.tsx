...
const CameraModal: React.FC<{ isOpen: boolean; onClose: () => void; onCapture: (base64: string) => void; }> = ({ isOpen, onClose, onCapture }) => {
    ...
    return (
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-[100]" onClick={onClose}>
            <div className="bg-white rounded-lg shadow-xl p-4 w-full max-w-lg m-4" onClick={e => e.stopPropagation()}>
                <h3 className="text-lg font-bold mb-2">拍照上传</h3>
                {error ? (
                    <p className="text-red-500">{error}</p>
                ) : (
                    <video ref={videoRef} autoPlay playsInline className="w-full rounded-md bg-slate-200 aspect-video object-cover"></video>
                )}
                <canvas ref={canvasRef} className="hidden"></canvas>
                <div className="mt-4 flex justify-end space-x-2">
                    <button onClick={onClose} className="px-4 py-2 bg-slate-200 text-slate-800 rounded-md">取消</button>
                    <button onClick={handleCapture} disabled={!!error} className="px-4 py-2 bg-[color:rgb(var(--color-primary))] text-white rounded-md disabled:bg-slate-400">拍摄</button>
                </div>
            </div>
        </div>
    );
...
const DecisionGuide: React.FC<DecisionGuideProps> = ({ distributions, learningPaths, setCurrentPage, setSelectedId }) => {
    ...
    const [contextFile, setContextFile] = useState<{ base64: {mimeType: string; data: string}; file: File } | null>(null);
...
The entire JSON response, including all keys and values, must be in Chinese.`;
            ...
            const responseSchema = {
                ...
                properties: {
                    ...
                    recommendedModels: {
                        type: Type.ARRAY, description: "一个包含2-3个推荐统计模型的数组。",
                        items: {
                            type: Type.OBJECT, properties: {
                                modelId: { type: Type.INTEGER }, modelName: { type: Type.STRING },
                                relevance: { type: Type.STRING, description: "解释为什么这个模型与解决用户的问题相关。" },
                            }, required: ["modelId", "modelName", "relevance"],
                        },
                    },
                    recommendedPaths: {
                        type: Type.ARRAY, description: "一个包含1-2个推荐学习路径的数组。",
                        items: {
                            type: Type.OBJECT, properties: {
                                pathId: { type: Type.STRING }, pathTitle: { type: Type.STRING },
                                relevance: { type: Type.STRING, description: "解释为什么这个学习路径有帮助。" },
                            }, required: ["pathId", "pathTitle", "relevance"],
                        },
                    },
                    nextSteps: {
                        type: Type.ARRAY, description: "一个包含3-5个具体后续步骤的列表。",
            ...
            setError(`分析失败。请尝试调整您的问题描述或稍后再试。(${e instanceof Error ? e.message : String(e)})`);
            setResult(null);
        } finally {
            stopLoading();
        }
...
- Your entire response must be in Chinese.`;
    ...
    return (
        <div className="animate-fade-in-up">
            <CameraModal isOpen={showCamera} onClose={() => setShowCamera(false)} onCapture={(img) => handleImageCapture(img, !!activeQuestion)} />
            <header className="mb-8 text-center">
                <h1 className="text-3xl sm:text-4xl font-extrabold text-[color:var(--color-text-base)] tracking-tight">AI 决策参谋</h1>
                <p className="text-lg text-[color:var(--color-text-muted)] mt-2 max-w-3xl mx-auto">描述您的业务挑战，AI 将为您连接到最相关的模型、学习路径和行动方案。</p>
            </header>
                 ...
                 <div className="bg-white p-4 sm:p-6 rounded-2xl shadow-lg border border-slate-200">
                    <div className="relative">
                        ...
                        />
                         <div className="absolute bottom-2 right-2 flex items-center space-x-1">
                            ...
                            <button onClick={() => fileInputRef.current?.click()} title="上传图片" className="p-2 rounded-full hover:bg-slate-200 text-slate-500 transition"><span className="material-symbols-outlined">upload_file</span></button>
                            <button onClick={() => setShowCamera(true)} title="拍照" className="p-2 rounded-full hover:bg-slate-200 text-slate-500 transition"><span className="material-symbols-outlined">photo_camera</span></button>
                            <button onClick={handleMicClick} title="语音输入" className={`p-2 rounded-full hover:bg-slate-200 text-slate-500 transition ${isRecording ? 'bg-red-100 !text-red-600 animate-pulse' : ''}`}><span className="material-symbols-outlined">{isRecording ? 'mic' : 'mic_none'}</span></button>
                        </div>
                    </div>
                    ...
                    {uploadedImage && (
                        <div className="mt-3 relative w-32 h-32 border rounded-lg p-1">
                            ...
                            <button onClick={() => setUploadedImage(null)} title="移除图片" className="absolute -top-2 -right-2 bg-slate-700 text-white rounded-full w-6 h-6 flex items-center justify-center text-sm hover:bg-red-600 transition">
                    ...
                    <div className="mt-3">
                        <button onClick={() => handleAnalyze()} disabled={isLoading} className="w-full bg-[color:rgb(var(--color-primary))] text-white font-bold py-2.5 px-6 rounded-lg hover:opacity-90 transition-opacity flex items-center justify-center space-x-2 disabled:bg-slate-400">
                           {isLoading ? <div className="w-5 h-5 border-2 border-white border-t-transparent rounded-full animate-spin"></div> : <span className="material-symbols-outlined">lightbulb</span>}
                           <span>{isLoading ? '正在分析...' : 'AI 分析 & 建议'}</span>
                        </button>
                    </div>
                    <div className="mt-4 pt-4 border-t border-slate-200/60">
                        <div className="flex justify-between items-center mb-2">
                            <h3 className="text-sm font-semibold text-slate-500">没有灵感？试试这些 AI 生成的问题：</h3>
                            <button onClick={fetchAiSuggestions} disabled={isFetchingSuggestions || isLoading} className="p-1 rounded-full hover:bg-slate-200 text-slate-500 transition disabled:opacity-50">
                                <span className={`material-symbols-outlined text-base ${isFetchingSuggestions ? 'animate-spin' : ''}`}>refresh</span>
                            </button>
                        </div>
                        <div className="space-y-2">
                            {(isFetchingSuggestions && aiSuggestions.length === 0) ? (
                                ...
                                aiSuggestions.map((suggestion, index) => (
                                    <button 
                                      key={index} 
                                      onClick={() => { setProblem(suggestion.question); handleAnalyze(suggestion.question); }} 
                                      disabled={isLoading} 
                                    ...
                                    >
                                        ...
                                        <div className="flex flex-wrap gap-1.5 mt-1.5">
                                            {suggestion.keywords.map(kw => <span key={kw} className="px-2 py-0.5 text-xs bg-slate-200 text-slate-600 rounded-full group-hover:bg-[color:rgb(var(--color-primary)/0.1)] group-hover:text-[color:rgb(var(--color-primary))]">{kw}</span>)}
                                        </div>
                                    </button>
                                ))
                            )}
                ...
                <div className="mt-8">
                    ...
                    {result && (
                         <div className="space-y-8 animate-fade-in-up">
                            <section>
                                <h2 className="text-xl font-bold text-[color:var(--color-text-base)] mb-3">AI 摘要与洞察</h2>
                                <p className="text-md text-[color:var(--color-text-muted)] bg-white p-4 rounded-xl border border-slate-200 shadow-sm">{result.problemSummary}</p>
                            </section>

                            <section>
                                <h2 className="text-xl font-bold text-[color:var(--color-text-base)] mb-3">推荐的数据模型</h2>
                                <div className="space-y-4">
                                    {result.recommendedModels.map((model: any) => (
                                        <button key={model.modelId} onClick={() => handleNavigation('models', model.modelId)} className="w-full text-left p-4 bg-white rounded-xl shadow-sm border border-slate-200 hover:shadow-md hover:border-[color:rgb(var(--color-primary)/0.5)] transition-all">
                                            <h3 className="font-bold text-[color:rgb(var(--color-primary))]">{model.modelName}</h3>
                                            <p className="text-sm text-[color:var(--color-text-muted)] mt-1">{model.relevance}</p>
                                        </button>
                                    ))}
                                </div>
                            </section>
                            ...
                            {result.recommendedPaths && result.recommendedPaths.length > 0 && (
                                <section>
                                    <h2 className="text-xl font-bold text-[color:var(--color-text-base)] mb-3">推荐的学习路径</h2>
                                    <div className="space-y-4">
                                        {result.recommendedPaths.map((path: any) => (
                                            <button key={path.pathId} onClick={() => handleNavigation('paths')} className="w-full text-left p-4 bg-white rounded-xl shadow-sm border border-slate-200 hover:shadow-md hover:border-[color:rgb(var(--color-primary)/0.5)] transition-all">
                                                <h3 className="font-bold text-[color:rgb(var(--color-primary))]">{path.pathTitle}</h3>
                                                <p className="text-sm text-[color:var(--color-text-muted)] mt-1">{path.relevance}</p>
                                            </button>
                                        ))}
                                    </div>
                                </section>
                            ...
                            <section>
                                <h2 className="text-xl font-bold text-[color:var(--color-text-base)] mb-3">建议的行动步骤</h2>
                                <ul className="list-none space-y-3">
                                    {result.nextSteps.map((step: string, index: number) => (
                                        <li key={index} className="flex items-start p-3 bg-white rounded-xl border border-slate-200 shadow-sm">
                                            <span className="material-symbols-outlined text-xl text-[color:rgb(var(--color-primary))] mr-3 mt-0.5">check_circle</span>
            ...
            <section className="mt-16 max-w-4xl mx-auto">
                <div className="text-center mb-8">
                    <h2 className="text-2xl font-extrabold text-[color:var(--color-text-base)]">常见业务问题导航</h2>
                    <p className="text-md text-[color:var(--color-text-muted)] mt-1">从常见业务挑战出发，快速找到对应的解决方案和数据模型。</p>
                </div>
                <div className="space-y-4">
                    {decisionGuideData.map(category => (
                        <details key={category.category} className="bg-white/50 border border-slate-200 p-3 rounded-xl transition-all duration-300 open:shadow-md open:bg-white" open={decisionGuideData.indexOf(category) === 0}>
                            <summary className="font-bold text-md text-[color:var(--color-text-base)] cursor-pointer flex items-center space-x-2 list-none">
                               <span className="material-symbols-outlined text-[color:rgb(var(--color-primary))]">{category.icon}</span>
            ...
            <section className="mt-16 max-w-4xl mx-auto">
                <div className="flex justify-center items-center gap-4 text-center mb-8">
                    <div>
                        <h2 className="text-2xl font-extrabold text-[color:var(--color-text-base)]">数据科学家如何理解医美业务</h2>
                        <p className="text-md text-[color:var(--color-text-muted)] mt-1">从这些问题开始，像数据科学家一样思考您的业务，并获得AI的深度分析。</p>
                    </div>
                     <button onClick={fetchDsQuestions} disabled={isLoadingDsQuestions || isGeneratingAnswer} className="p-2 rounded-full hover:bg-slate-200 text-slate-500 transition disabled:opacity-50" title="刷新问题列表">
                        <span className={`material-symbols-outlined text-xl ${isLoadingDsQuestions ? 'animate-spin' : ''}`}>refresh</span>
                    </button>
                </div>
                ...
                {isLoadingDsQuestions ? (
                    ...
                    <div className="space-y-3">
                        {dsQuestions.map(category => (
                            <details key={category.category} className="bg-white/50 border border-slate-200 p-3 rounded-xl transition-all duration-300 open:shadow-md open:bg-white" open={dsQuestions.indexOf(category) <= 1}>
                                <summary className="font-bold text-md text-[color:var(--color-text-base)] cursor-pointer flex items-center space-x-2 list-none">
                                   <span className="material-symbols-outlined text-[color:rgb(var(--color-primary))]">{category.icon}</span>
                                   <span>{category.category}</span>
                                </summary>
                                <div className="mt-4 space-y-2">
                                    {category.questions.map((question, index) => (
                                        <div key={index} ref={activeQuestion?.question === question ? activeQuestionRef : null}>
                                            <button onClick={() => handleQuestionClick(category.category, question)} className="w-full text-left p-3 rounded-lg hover:bg-slate-50 transition">
                                            ...
                                            {activeQuestion?.question === question && (
                                                <div className="p-4 bg-slate-50 border-t border-slate-200 animate-fade-in-up">
                                                    {isGeneratingAnswer ? (
                                                        ...
                                                        <div>
                                                            <article className="prose prose-sm max-w-full"><ReactMarkdown>{generatedAnswer}</ReactMarkdown></article>
                                                            <button onClick={() => setActiveQuestion(null)} className="mt-4 text-xs font-semibold text-[color:rgb(var(--color-primary))]">收起回答</button>
                                                        </div>
                                                    ) : (
                                                        <div className="space-y-3">
                                                            <p className="text-sm font-medium text-slate-600">请提供更多背景信息或上传相关文件 (可选):</p>
                                                            <textarea value={userContext} onChange={e => setUserContext(e.target.value)} rows={3} className="w-full text-sm p-2 border rounded-md" placeholder="例如：我们是一家位于市中心的皮肤管理诊所..."></textarea>
                                                            {contextFile && (
                                                                <div className="text-xs flex items-center space-x-2 bg-slate-200 p-1.5 rounded-md">
                                                                    ...
                                                                    <button onClick={() => setContextFile(null)}><span className="material-symbols-outlined !text-base">close</span></button>
                                                                </div>
                                                            )}
                                                            <div className="flex items-center space-x-2">
                                                                <input type="file" ref={fileInputRef} onChange={(e) => handleFileChange(e, true)} className="hidden"/>
                                                                <button onClick={() => fileInputRef.current?.click()} className="text-xs flex items-center space-x-1 px-2 py-1 bg-white border rounded-md hover:bg-slate-100"><span className="material-symbols-outlined !text-sm">upload_file</span><span>上传文件</span></button>
                                                                <button onClick={() => setShowCamera(true)} className="text-xs flex items-center space-x-1 px-2 py-1 bg-white border rounded-md hover:bg-slate-100"><span className="material-symbols-outlined !text-sm">photo_camera</span><span>拍照</span></button>
                                                            </div>
                                                            <button onClick={handleGenerateAnswer} className="w-full py-2 bg-[color:rgb(var(--color-primary))] text-white font-semibold rounded-md hover:opacity-90 transition">提交并获取AI分析</button>
                                                        </div>
                                                    )}
                                                </div>
                                            )}
