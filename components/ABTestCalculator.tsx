            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 p-4 border-b border-slate-200">
                ...
                <div className="space-y-2">
                    ...
                    <div>
                        ...
                        <input type="number" value={visitorsA} onChange={e => setVisitorsA(Math.max(1, +e.target.value))} className="w-full p-1 border rounded" />
                    </div>
                    <div>
                        <label className="text-sm font-medium">转化数</label>
                        <input type="number" value={conversionsA} onChange={e => setConversionsA(Math.max(0, Math.min(visitorsA, +e.target.value)))} className="w-full p-1 border rounded" />
                    </div>
                </div>
                {/* Variant B */}
                <div className="space-y-2">
                    <h3 className="font-bold text-center" style={{ color: `rgb(${themeColors.primaryColor})` }}>方案 B</h3>
                    <div>
                        <label className="text-sm font-medium">访客数</label>
                        <input type="number" value={visitorsB} onChange={e => setVisitorsB(Math.max(1, +e.target.value))} className="w-full p-1 border rounded" />
                    </div>
                    <div>
                        <label className="text-sm font-medium">转化数</label>
                        <input type="number" value={conversionsB} onChange={e => setConversionsB(Math.max(0, Math.min(visitorsB, +e.target.value)))} className="w-full p-1 border rounded" />
                    </div>
                </div>
            </div>
            ...
            <div className="p-4 text-center">
                ...
                {result && !isLoading && (
                    <div className="animate-fade-in-up">
                        <p className="text-lg">
                            B方案优于A方案的概率是: 
                            <strong className="text-2xl mx-2" style={{ color: `rgb(${themeColors.primaryColor})` }}>
                                {result.probBbetterThanA.toFixed(2)}%
                            </strong>
                        </p>
                        <p className="text-sm text-slate-500 mt-1">
                            预期转化率提升约 <strong>{result.expectedLift.toFixed(2)}%</strong> (从 {result.expectedA.toFixed(2)}% 到 {result.expectedB.toFixed(2)}%)
                        </p>
                    </div>
                )}
            </div>
