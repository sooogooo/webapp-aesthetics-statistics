...
const AiDesigner: React.FC = () => {
  ...
  const [selectedTemplateId, setSelectedTemplateId] = useState<string>(promptData[0].templates[0].id);
  ...
  const selectedTemplate = useMemo(() => {
    return promptData
      .find(c => c.id === selectedCategory)?.templates
      .find(t => t.id === selectedTemplateId) || promptData[0].templates[0];
  ...
  const AspectRatioButton: React.FC<{ value: typeof aspectRatio; label: string, disabled?: boolean }> = ({ value, label, disabled }) => (
    <button
  ...
  return (
    <div className="animate-fade-in-up">
       <header className="mb-8">
        <h1 className="text-3xl sm:text-4xl font-extrabold text-[color:var(--color-text-base)] tracking-tight">AI 美学营销设计师</h1>
        <p className="text-lg text-[color:var(--color-text-muted)] mt-2">为您的医美业务，生成高度定制化、专业级的营销与教育视觉素材。</p>
      </header>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <div className="lg:col-span-4 space-y-4">
            <div className="space-y-4 p-4 bg-white rounded-xl shadow-sm border border-slate-200">
              <div>
                <label className="text-sm font-bold text-[color:var(--color-text-base)]">1. 选择创作主题</label>
                <select onChange={(e) => { setSelectedCategory(e.target.value); setSelectedTemplateId(promptData.find(c=>c.id === e.target.value)!.templates[0].id) }} value={selectedCategory} className="w-full mt-2 p-2 border border-slate-300 rounded-md bg-white">
                  {promptData.map(cat => <option key={cat.id} value={cat.id}>{cat.name}</option>)}
                </select>
              </div>
              <div>
                <label className="text-sm font-bold text-[color:var(--color-text-base)]">2. 选择提示词模板</label>
                <div className="space-y-1 mt-2 max-h-40 overflow-y-auto pr-2">
                  {promptData.find(c => c.id === selectedCategory)?.templates.map(template => (
                    <button key={template.id} onClick={() => setSelectedTemplateId(template.id)} className={`w-full text-left p-2 rounded-md text-xs transition-colors ${selectedTemplateId === template.id ? 'bg-[color:rgb(var(--color-primary)/0.1)]' : 'hover:bg-slate-100'}`}>
                       {template.title}
                    </button>
                  ))}
                </div>
            ...
            <div className="p-4 bg-white rounded-xl shadow-sm border border-slate-200">
               <label className="text-sm font-bold text-[color:var(--color-text-base)]">3. 添加微调模块</label>
               <div className="space-y-3 mt-2">
                  {Object.entries(modifierChips).map(([category, chips]) => (
                     <div key={category}>
                        <h4 className="text-xs font-semibold text-slate-500 capitalize mb-1.5">{category}</h4>
                        <div className="flex flex-wrap gap-1.5">
                            {chips.map(chip => <ModifierChip key={chip} modifier={chip} />)}
                        </div>
                     </div>
                  ))}
               </div>
             ...
             <div className="p-4 bg-white rounded-xl shadow-sm border border-slate-200">
                <label className="text-sm font-bold text-[color:var(--color-text-base)]">4. (可选) 上传参考图</label>
                <p className="text-xs text-slate-500 mt-1">AI将尝试模仿参考图的色彩、构图或风格。</p>
                {referenceImage ? (
                <div className="mt-2 relative">
                    <img src={`data:${referenceImage.file.type};base64,${referenceImage.base64}`} alt="Reference" className="w-full rounded-md object-contain max-h-40 border" />
                    <button
            ...
            <div className="p-4 bg-white rounded-xl shadow-sm border border-slate-200">
               <label className="text-sm font-bold text-[color:var(--color-text-base)]">5. (可选) 添加自定义描述</label>
               <textarea value={customPrompt} onChange={(e) => setCustomPrompt(e.target.value)} placeholder="例如：背景中有绿植" className="w-full mt-2 p-2 h-16 border rounded-md text-sm" />
            </div>

            <div className="p-4 bg-white rounded-xl shadow-sm border border-slate-200">
              <label className="text-sm font-bold text-[color:var(--color-text-base)]">6. 选择图片比例</label>
              {referenceImage && <p className="text-xs text-amber-600 mt-1">参考图模式下不支持自定义比例，将生成1:1方图。</p>}
            ...
            <button
              ...
              >
                {isLoading ? <div className="w-5 h-5 border-2 border-white border-t-transparent rounded-full animate-spin"></div> : <span className="material-symbols-outlined">auto_awesome</span>}
        ...
        <div className="lg:col-span-8 flex flex-col gap-8">
            <div className="relative bg-white rounded-xl shadow-sm border border-slate-200 p-4 min-h-[20rem] flex items-center justify-center flex-col" style={{ aspectRatio: aspectRatio.replace(':', ' / ') }}>
              {isLoading && (
              ...
              {generatedImage && !isLoading && (
                <>
                    <img src={`data:image/png;base64,${generatedImage}`} alt={finalPrompt} className="w-full h-full object-contain rounded-lg"/>
                    {history.length > 0 && (
                         <div className="absolute bottom-2 right-2 bg-white/80 backdrop-blur-sm p-1 rounded-lg">
                            <Feedback contentId={`designer-${history[0]?.id}`} contentType="generated_image" promptText="" />
                        </div>
                    )}
                </>
              )}
             ...
             <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-4">
              <h3 className="text-sm font-bold text-[color:var(--color-text-base)] mb-2">最终提示词预览</h3>
              <p className="text-xs p-2 bg-slate-50 rounded-md text-slate-600 h-24 overflow-y-auto">{finalPrompt}</p>
            </div>
            ...
            <div>
              ...
              {history.length > 0 ? (
                <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                  {history.map(item => (
                    <button key={item.id} onClick={() => loadFromHistory(item)} className="group relative aspect-square rounded-lg overflow-hidden border-2 border-transparent hover:border-[color:rgb(var(--color-primary))] transition-all">
