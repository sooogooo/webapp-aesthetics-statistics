...
const SettingOptionButton = ({ value, currentValue, onClick, label }: SettingOptionButtonProps) => (
  <button
    onClick={onClick}
    className={`px-3 py-1.5 rounded-full text-sm transition ${value === currentValue ? 'bg-[color:rgb(var(--color-primary))] text-white' : 'bg-[color:var(--color-bg-muted)] hover:bg-[color:var(--color-border)]'}`}
  >
    {label}
  </button>
);
...
const SettingsModal: React.FC<SettingsModalProps> = ({ isOpen, setIsOpen }) => {
  ...
  return (
    ...
    >
      <div
        className="bg-white rounded-2xl shadow-xl w-full max-w-sm m-4"
        ...
        <div className="p-5 space-y-6">
          ...
          <div>
            ...
            <div className="flex space-x-2">
              <SettingOptionButton
                ...
                onClick={() => updateSetting('fontSize', 'font-size-sm')}
                label="小"
              />
              <SettingOptionButton
                value="font-size-md"
                currentValue={settings.fontSize}
                onClick={() => updateSetting('fontSize', 'font-size-md')}
                label="中"
              />
              <SettingOptionButton
                value="font-size-lg"
                currentValue={settings.fontSize}
                onClick={() => updateSetting('fontSize', 'font-size-lg')}
                label="大"
              />
            </div>
          </div>
          ...
          <div>
            ...
            <div className="flex space-x-2">
              <SettingOptionButton
                value="轻松幽默"
                currentValue={settings.aiStyle}
                onClick={() => updateSetting('aiStyle', '轻松幽默')}
                label="轻松幽默"
              />
              <SettingOptionButton
                value="标准日常"
                currentValue={settings.aiStyle}
                onClick={() => updateSetting('aiStyle', '标准日常')}
                label="标准日常"
              />
              <SettingOptionButton
                value="科学严谨"
                currentValue={settings.aiStyle}
                onClick={() => updateSetting('aiStyle', '科学严谨')}
                label="科学严谨"
              />
            </div>
          </div>
          ...
          <div>
            ...
            <div className="flex space-x-2">
              <SettingOptionButton
                value="简约"
                currentValue={settings.aiLength}
                onClick={() => updateSetting('aiLength', '简约')}
                label="简约"
              />
              <SettingOptionButton
                value="标准"
                currentValue={settings.aiLength}
                onClick={() => updateSetting('aiLength', '标准')}
                label="标准"
              />
              <SettingOptionButton
                value="详细"
                currentValue={settings.aiLength}
                onClick={() => updateSetting('aiLength', '详细')}
                label="详细"
              />
            </div>
          </div>
