...
const KPICard: React.FC<{ icon: string; title: string; value: string; trend: string; modelId: number; onClick: (id: number) => void }> = ({ icon, title, value, trend, modelId, onClick }) => (
  <button onClick={() => onClick(modelId)} className="bg-white p-4 rounded-xl shadow-sm border border-slate-200 hover:shadow-lg hover:border-[color:rgb(var(--color-primary))] transition-all duration-300 text-left w-full">
    <div className="flex justify-between items-start">
      <div className="bg-[color:rgb(var(--color-primary)/0.1)] p-2 rounded-full">
        <span className="material-symbols-outlined text-xl text-[color:rgb(var(--color-primary))]">{icon}</span>
      </div>
      <span className={`text-xs font-semibold px-2 py-0.5 rounded-full ${trend.startsWith('+') || trend === 'Top 1' ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>{trend}</span>
    </div>
...
export const Dashboard: React.FC<DashboardProps> = ({ distributions, setCurrentPage, setSelectedId }) => {
  ...
  const kpiData = useMemo(() => {
      ...
      const newCustTrend = newCustomers >= 325 ? `+${(((newCustomers/325)-1)*100).toFixed(0)}%` : `-${((1-(newCustomers/325))*100).toFixed(0)}%`;
      ...
      const clvTrend = clv >= 85400 ? `+${(((clv/85400)-1)*100).toFixed(1)}%` : `-${((1-(clv/85400))*100).toFixed(1)}%`;
  ...
  const decisionGuideSnippet = useMemo(() => {
    return decisionGuideData[0].problems[Math.floor(Math.random() * decisionGuideData[0].problems.length)];
  }, []);

  return (
    <div className="space-y-10">
      <header>
        <h1 className="text-3xl sm:text-4xl font-extrabold text-[color:var(--color-text-base)] tracking-tight">决策仪表盘</h1>
        <p className="text-lg text-[color:var(--color-text-muted)] mt-2">欢迎回来！这是您的医美业务数据洞察中心。</p>
      </header>
      
      <section>
        <h2 className="text-xl font-bold text-[color:var(--color-text-base)] mb-4">核心业务指标 (模拟)</h2>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            <KPICard icon="group_add" title="本月新客" {...kpiData.newCustomers} onClick={(id) => handleNavigation('models', id)} />
            <KPICard icon="trending_down" title="高价值流失风险" {...kpiData.churnRisk} onClick={(id) => handleNavigation('models', id)} />
            <KPICard icon="shopping_basket" title="Top 1 项目关联" {...kpiData.association} onClick={(id) => handleNavigation('models', id)} />
            <KPICard icon="payments" title="预测客户终身价值" {...kpiData.clv} onClick={(id) => handleNavigation('models', id)} />
        </div>
      </section>
      ...
      <section>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {recommendedModels.map(rec => {
            ...
            return (
              <button key={model.id} onClick={() => handleNavigation('models', model.id)} className="bg-white p-4 rounded-xl shadow-sm border border-slate-200 hover:shadow-lg hover:border-[color:rgb(var(--color-primary))] transition-all duration-300 text-left w-full h-full flex flex-col justify-between">
                <div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <section className="lg:col-span-2">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <button onClick={() => handleNavigation('paths')} className="group p-4 bg-white rounded-xl shadow-sm border border-slate-200 text-left hover:bg-[color:rgb(var(--color-primary)/0.05)] transition">
                    <span className="material-symbols-outlined text-3xl text-[color:rgb(var(--color-primary))]">school</span>
                    <h3 className="font-bold mt-2 text-lg">学习路径</h3>
                    <p className="text-sm text-slate-500 mt-1">从您的角色出发，系统化学习解决实际问题的模型组合。</p>
                </button>
                <button onClick={() => handleNavigation('guide')} className="group p-4 bg-white rounded-xl shadow-sm border border-slate-200 text-left hover:bg-[color:rgb(var(--color-primary)/0.05)] transition">
                    <span className="material-symbols-outlined text-3xl text-[color:rgb(var(--color-primary))]">lightbulb</span>
                    <h3 className="font-bold mt-2 text-lg">决策参谋</h3>
                    <p className="text-sm text-slate-500 mt-1">输入您的业务挑战，让AI为您匹配最合适的解决方案。</p>
                </button>
                 <button onClick={() => handleNavigation('designer')} className="group p-4 bg-white rounded-xl shadow-sm border border-slate-200 text-left hover:bg-[color:rgb(var(--color-primary)/0.05)] transition">
                    <span className="material-symbols-outlined text-3xl text-[color:rgb(var(--color-primary))]">palette</span>
                    <h3 className="font-bold mt-2 text-lg">AI 设计室</h3>
                    <p className="text-sm text-slate-500 mt-1">生成符合医美美学的高质量营销图片和设计素材。</p>
                </button>
                 <button onClick={() => handleNavigation('copilot')} className="group p-4 bg-white rounded-xl shadow-sm border border-slate-200 text-left hover:bg-[color:rgb(var(--color-primary)/0.05)] transition">
                    <span className="material-symbols-outlined text-3xl text-[color:rgb(var(--color-primary))]">query_stats</span>
                    <h3 className="font-bold mt-2 text-lg">智能统计</h3>
                    <p className="text-sm text-slate-500 mt-1">上传您的业务数据，与AI对话，获得定制化的数据分析。</p>
                </button>
            </div>
        </section>
        
        <section>
            <h2 className="text-xl font-bold text-[color:var(--color-text-base)] mb-4">决策导航精选</h2>
             <div className="bg-white p-4 rounded-xl shadow-sm border border-slate-200 h-full">
                ...
                <div className="flex flex-wrap gap-2">
                    {decisionGuideSnippet.tools.map(tool => (
                        <button key={tool.id} onClick={() => handleNavigation('models', tool.id)} className="px-2.5 py-1 bg-[color:rgb(var(--color-primary)/0.1)] text-[color:rgb(var(--color-primary))] text-xs font-semibold rounded-full hover:bg-[color:rgb(var(--color-primary)/0.2)] transition">
                            {tool.name}
                        </button>
                    ))}
                </div>
