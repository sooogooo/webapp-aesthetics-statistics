  ...
  return (
    <div className="space-y-8 animate-fade-in-up">
        ...
        <section>
          <div className="bg-sky-50/80 border border-sky-200 p-4 rounded-xl flex flex-col sm:flex-row items-center justify-between gap-4">
            <div className="flex items-center space-x-3">
              <span className="material-symbols-outlined text-3xl text-sky-600 p-2 bg-sky-100 rounded-full">
                function
              </span>
              <div>
                <h3 className="font-bold text-sky-800">探索核心参数: μ 和 σ</h3>
                <p className="text-sm text-sky-700 mt-1">
                  了解均值 (μ) 和标准差 (σ) 如何共同决定正态分布的“山峰”位置和“胖瘦”。
                </p>
              </div>
            </div>
            <button
      ...
      <section className="bg-white p-6 rounded-xl shadow-sm border border-slate-200">
        <h2 className="text-xl font-bold mb-4 text-[color:var(--color-text-base)]">
          核心洞察 (Takeaway)
        </h2>
        <div className="border-l-4 border-[color:rgb(var(--color-primary))] pl-4">
          <p className="text-md font-medium text-[color:var(--color-text-muted)] italic">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {distribution.relatedModels.map((model) => {
              ...
              return (
                ...
                >
                  <p className="font-bold text-[color:rgb(var(--color-primary))] hover:underline">
                    {model.name}
                  </p>
                  <p className="text-sm text-slate-600 mt-1">{model.reason}</p>
                  {relatedDist && (
                    <p className="text-xs text-slate-400 mt-2 line-clamp-2">
                      {relatedDist.description}
                    </p>
                  )}
                </button>
              );
        ...
        <div className="border-b border-slate-200">
          <nav className="-mb-px flex space-x-6" aria-label="Tabs">
            <button
              ...
              className={`whitespace-nowrap py-3 px-1 border-b-2 font-semibold text-sm ${activeTab === 'application' ? 'border-[color:rgb(var(--color-primary))] text-[color:rgb(var(--color-primary))]' : 'border-transparent text-slate-500 hover:text-slate-700 hover:border-slate-300'}`}
            >
              应用场景
            </button>
            <button
              onClick={() => setActiveTab('analysis')}
              className={`whitespace-nowrap py-3 px-1 border-b-2 font-semibold text-sm ${activeTab === 'analysis' ? 'border-[color:rgb(var(--color-primary))] text-[color:rgb(var(--color-primary))]' : 'border-transparent text-slate-500 hover:text-slate-700 hover:border-slate-300'}`}
            >
              AI 深度解读
            </button>
            <button
              onClick={() => setActiveTab('data')}
              className={`whitespace-nowrap py-3 px-1 border-b-2 font-semibold text-sm ${activeTab === 'data' ? 'border-[color:rgb(var(--color-primary))] text-[color:rgb(var(--color-primary))]' : 'border-transparent text-slate-500 hover:text-slate-700 hover:border-slate-300'}`}
            >
              模拟数据
            </button>
            <button
              onClick={() => setActiveTab('details')}
              className={`whitespace-nowrap py-3 px-1 border-b-2 font-semibold text-sm ${activeTab === 'details' ? 'border-[color:rgb(var(--color-primary))] text-[color:rgb(var(--color-primary))]' : 'border-transparent text-slate-500 hover:text-slate-700 hover:border-slate-300'}`}
            >
              技术细节
            </button>
          </nav>
            ...
            <div className="space-y-4">
              {distribution.application.map((app, index) => {
                ...
                return (
                  <div
                    key={index}
                    className="p-4 bg-white rounded-lg shadow-sm border border-slate-200/80"
                  >
                    <p className="text-slate-700">{app}</p>
                    {enhancedApp && (
                      <div className="mt-3 pt-3 border-t border-dashed border-slate-200">
                        <p className="text-xs font-semibold text-sky-600 mb-1">AI 优化思路:</p>
                        <p className="text-sm text-sky-800 bg-sky-50/80 p-2 rounded-md">
                          {enhancedApp.enhancement}
                        </p>
                      </div>
                    )}
                  </div>
              ...
              <div className="mt-8 pt-6 border-t border-dashed border-slate-200">
                <div className="flex justify-between items-center">
                  <h3 className="text-lg font-bold text-[color:var(--color-text-base)] flex items-center">
                    <span className="material-symbols-outlined text-xl text-[color:rgb(var(--color-primary))] align-bottom mr-2">
                      auto_awesome
                    </span>
                    AI 生成案例
                  </h3>
                  ...
                  >
                    ...
                    <span>
                      {isGeneratingCases ? '生成中...' : aiGeneratedCases ? '重新生成' : '点击生成'}
                    </span>
                  </button>
                </div>
                ...
                <div className="mt-4 space-y-4">
                  {isGeneratingCases && (
                    <div className="p-4 bg-slate-50 rounded-lg text-center text-slate-500 text-sm">
                      <p>
                        AI 正在根据 <strong>{distribution.name}</strong>{' '}
                        的特点，为您构思创新的商业应用场景...
                      </p>
                    </div>
                  )}
                  {aiGeneratedCases &&
                    aiGeneratedCases.map((caseText, index) => (
                      <div
                        key={index}
                        className="p-4 bg-white rounded-lg shadow-sm border border-slate-200/80 animate-fade-in-up"
                      >
          ...
          {activeTab === 'analysis' && deepAnalysis && (
            <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-200">
              ...
              <div className="pt-6 border-t border-slate-200 mt-6 not-prose">
                <Feedback
                  contentId={`deep-analysis-${distribution.id}`}
            ...
            <div className="bg-white p-4 rounded-xl shadow-sm border border-slate-200">
              <div className="flex justify-between items-center mb-2">
                ...
                <button
                  ...
                  className="text-xs font-semibold text-[color:rgb(var(--color-primary))]"
                >
                  下载CSV
                </button>
              </div>
          ...
          {activeTab === 'details' && (
            <div className="space-y-4">
              <div className="p-4 bg-white rounded-lg shadow-sm border border-slate-200/80">
                ...
                <p className="mt-2 text-md text-[color:var(--color-text-muted)]">
