...
const Feedback: React.FC<FeedbackProps> = ({
  ...
  const buttonClass =
    variant === 'light' ? 'text-slate-500 hover:bg-slate-100' : 'text-slate-400 hover:bg-slate-700';
  const goodSelectedClass =
    variant === 'light' ? 'bg-green-100 text-green-700' : 'bg-green-800/50 text-green-300';
  const badSelectedClass =
    variant === 'light' ? 'bg-red-100 text-red-700' : 'bg-red-800/50 text-red-300';
  const submittedClass = variant === 'light' ? 'text-green-600' : 'text-green-400';

  if (submitted) {
    return (
      <div className={`text-xs ${submittedClass} animate-fade-in-up ${className}`}>
        感谢您的反馈！
      </div>
    );
  }
