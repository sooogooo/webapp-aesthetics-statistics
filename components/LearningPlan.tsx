      ...
      return (
        <div className="p-4 sm:p-6 md:p-8 max-w-4xl mx-auto w-full">
          <button
            ...
            className="flex items-center space-x-1 text-sm text-slate-500 hover:text-[color:rgb(var(--color-primary))] mb-4"
          >
          ...
          <div className="bg-white p-6 rounded-xl shadow-lg border border-slate-200">
            {isLoading && (
              <div className="text-center p-10">
                ...
                <p className="mt-4 font-semibold text-[color:var(--color-text-base)]">
                  AI 正在为您定制学习计划...
                </p>
      ...
      <div className="p-4 sm:p-6 md:p-8">
        <div className="text-center max-w-3xl mx-auto">
          ...
          <p className="text-lg text-[color:var(--color-text-muted)] mt-4">
            请根据您的学习目标和基础，选择一个最适合您的学习计划类型。AI将为您生成一份个性化的学习蓝图。
          </p>
        </div>
        <div className="mt-10 grid grid-cols-1 lg:grid-cols-3 gap-6 max-w-6xl mx-auto">
          {planCategories.map((category) => (
            <div
              key={category.id}
              className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200 flex flex-col hover:shadow-xl hover:border-[color:rgb(var(--color-primary)/0.5)] transition-all duration-300 transform hover:-translate-y-1"
            >
              <div className="flex items-center space-x-3 mb-3">
                <span className="material-symbols-outlined text-2xl p-3 bg-[color:var(--color-bg-muted)] rounded-full text-[color:rgb(var(--color-primary))]">
                  {category.icon}
                </span>
                <h2 className="font-bold text-xl text-[color:var(--color-text-base)]">
                  {category.title}
                </h2>
              </div>
              <p className="text-sm text-[color:var(--color-text-muted)] flex-grow">
                {category.description}
              </p>
              <p className="text-xs font-semibold text-[color:var(--color-text-muted)] mt-4 mb-4">
                {category.duration}
              </p>
              <button
                onClick={() => generatePlan(category)}
                className="w-full py-2.5 bg-[color:rgb(var(--color-primary))] text-white font-semibold rounded-lg hover:opacity-90 transition"
              >
                生成学习计划
              </button>
            </div>
