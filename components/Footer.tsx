...
const Footer: React.FC<FooterProps> = ({ currentPage, setCurrentPage }) => {
    const navItemClass = (page: Page) => 
        `flex flex-col items-center justify-center space-y-1 w-full transition-colors duration-200 ${
            currentPage === page ? 'text-[color:rgb(var(--color-primary))] font-bold' : 'text-[color:var(--color-text-muted)] hover:text-[color:var(--color-text-base)] font-medium'
        }`;
    ...
    return (
        ...
        <div className="text-center text-xs text-[color:var(--color-text-muted)] opacity-60 py-4 px-4 mt-0 md:mt-8 mb-16 md:mb-0 space-y-1">
            ...
            <a href="https://beian.miit.gov.cn/" target="_blank" rel="noopener noreferrer" className="hover:text-[color:rgb(var(--color-primary))]">
                渝 ICP 备 2024023473 号
            </a>
            <div className="flex justify-center pt-2">
                 <img src="https://docs.bccsw.cn/logo.png" alt="系统Logo" className="h-12 w-auto object-contain"/>
            </div>
        </div>
        </>
    );
